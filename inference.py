    ...
    def observe(self, observation, gameState):
        ...
        * When a ghost is captured by Pacman, all beliefs should be
        ...
        for p in self.legalPositions:
            ...
            #If it is possible to have observed noisyDistance given the true distance
            if emissionModel[trueDistance] > 0:
                allPossible[p] = emissionModel[trueDistance]*self.beliefs[p]
        
        if noisyDistance == None:#The ghost has been captured
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for position in self.legalPositions:
            for i in range(0, self.numParticles / len(self.legalPositions)):
                #Particles is evenly distributed across positions 
    ...
    def observe(self, observation, gameState):
        ...
        #Check if the ghost has been captures
        if noisyDistance == None:
            for i in range(0,self.numParticles):
                self.particles[i] = self.getJailPosition()
        else:
            allPossible = self.getBeliefDistribution()
            #Calculate particles' possibilities
            for postion in self.legalPositions:
                trueDistance = util.manhattanDistance(postion, pacmanPosition)
                allPossible[postion] *= emissionModel[trueDistance]

            # Check if all particles receive 0 weight
            ...
            if all_weight_zero:
                #Recreate particles from the prior distribution
                self.initializeUniformly(gameState)
            else:
                #Generate a sample from the belief distribution
                new_sample = []
                for i in range(0,self.numParticles):
                    new_sample.append(util.sample(allPossible))
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        new_particles = []
        # Loop over all old positions of particles
        for particle in self.particles:
            # Get the distibution of current location
    ...
    def getBeliefDistribution(self):
        ...
        #Count the number of occurrence for each particle
        for particle in self.particles:
            allPossible[particle] +=1
        
        #Converts it into a belief distribution 
        allPossible.normalize()
        return allPossible
        "*** END YOUR CODE HERE ***"
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        # Computes the cartesian product of legalPositions 
        ...
        i = 0
        while i < self.numParticles:# The number of particles
    ...
    def observeState(self, gameState):
        ...
        for idx, particle in enumerate(self.particles):
            ...
            for ghost in range(self.numGhosts):
                if noisyDistances[ghost] is None:#This ghost has been captured
                    particle = self.getParticleWithGhostInJail(particle, ghost)#Put it in jail
                else:
                    currDistance = util.manhattanDistance(particle[ghost], pacmanPosition)
                    probability *= emissionModels[ghost][currDistance]
            allPossible[particle] += probability

        # Check whether all particles get weight 0
        all_weight_zero = True
        for i in allPossible:
            if allPossible[i] != 0:
                all_weight_zero = False
        ...
        if all_weight_zero:
            # Generated new particles from the initial prior distribution
            self.initializeParticles()
        else:
            new_particles = []
